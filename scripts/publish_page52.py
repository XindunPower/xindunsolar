#!/usr/bin/env python3
"""Publish xindun-power news page 52 articles to Spanish WordPress (cat 89)."""
...
DUOMI_MAP = {
    (0, 1): (
        "Translate waveform labels only to Spanish: "
        "'Phase 1'→'Fase 1'; 'Phase 2'→'Fase 2'; 'Phase3'→'Fase 3'. "
        "Keep axis numbers, degree marks, and all lines unchanged. Do not redesign."
    ),
    (3, 1): (
        "Translate overlay text only: 'pcs'→'uds'. "
        "Keep all voltage values (48vdc/96vdc/192vdc, 12v), product photos, "
        "battery arrangement, arrows, and logo unchanged."
    ),
    (4, 0): (
        "Translate diagram labels only to Spanish: "
        "'Power Grid'→'Red eléctrica'; "
        "'Generator'→'Generador'; "
        "'Hybrid Solar Inverter'→'Inversor solar híbrido'; "
        "'Solar Panels'→'Paneles solares'; "
        "'Batteries'→'Baterías'; "
        "'AC Loads'→'Cargas CA'; "
        "'Household Device'→'Dispositivos del hogar'; "
        "'Office Equipment'→'Equipos de oficina'; "
        "'Industrial Equipment'→'Equipos industriales'. "
        "Keep product image, wiring, icons, and layout unchanged."
    ),
}
