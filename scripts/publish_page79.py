#!/usr/bin/env python3
"""Publish xindun-power news page 79 articles to Spanish WordPress (cat 89)."""
...
# Article 0: DC/AC waveform diagram with DC, AC labels + inverter product photo.
# Article 1: product photo in RV setting (casing labels only) — compress only.
# Article 2: UPS working-principle circuit diagram with English labels.
# Article 3: residential rooftop solar photo — compress only.
# Article 4: solar farm photo — compress only.
base.DUOMI_MAP = {
    (0, 0): (
        "Translate diagram labels to Spanish: DC and AC. "
        "Keep waveform graphs, axes, inverter product photo, "
        "and white background layout unchanged."
    ),
    (2, 0): (
        "Translate diagram labels to Spanish: Non Inverting Input, Inverting Input, "
        "Output, and ESD. Keep circuit symbols, transistor/resistor/capacitor layout, "
        "current values (μA), VCC/VEE notation, and schematic composition unchanged."
    ),
}

TITLE_OVERRIDES = {
    "How do 12 volt power inverters convert DC electricity to AC?": (
        "¿Cómo convierten los inversores de 12 voltios la electricidad CC en CA?"
    ),
    "The use of car power inverter charger for rv/truck": (
        "El uso del cargador inversor de corriente para autocaravana/camión"
    ),
    "Backup uninterruptible power supply working principle": (
        "Principio de funcionamiento de la fuente de alimentación ininterrumpida de respaldo"
    ),
    "How to determine the installed capacity of solar energy power system at home?": (
        "¿Cómo determinar la capacidad instalada del sistema de energía solar en el hogar?"
    ),
    "Advantages and disadvantages of solar photovoltaic power station": (
        "Ventajas y desventajas de la central fotovoltaica solar"
    ),
}
...
ALT_OVERRIDES = {
    ...
    "determine the installed capacity of solar energy power system at home": (
        "Determinar la capacidad instalada del sistema de energía solar en el hogar"
    ),
    "Advantages and disadvantages of solar photovoltaic power station": (
        "Ventajas y desventajas de la central fotovoltaica solar"
    ),
...
def run_duomi_relaxed_ratio(
    ...
    # Wide schematics (e.g. 694x378 UPS diagram) legitimately raise diff when labels translate.
    if orig_ratio > 2.0:
        diff_limit = 50.0
    elif orig_ratio > 1.7:
        diff_limit = 55.0
