#!/usr/bin/env python3
"""Publish xindun-power news page 81 articles to Spanish WordPress (cat 89)."""
...
# Article 0–1: already published on prior pages (pages 78–79).
# Article 2: product/installation photos + customer review screenshot with English UI text.
# Article 3: product group photo (casing labels only) — compress only.
# Article 4: UPS product photo — compress only.
base.DUOMI_MAP = {
    (2, 2): (
        "Translate the English customer review UI text to Spanish: supplier ratings, "
        "product name, star labels, and review comment. Keep the screenshot layout, "
        "product thumbnail, stars, and composition unchanged."
    ),
}
...
TITLE_OVERRIDES = {
    ...
    "Xindun most praised solar generator kit for fridge/cabin/apartment": (
        "Kit generador solar más elogiado de Xindun para nevera/cabaña/apartamento"
    ),
    "Main equipment of photovoltaic off-grid system": (
        "Equipo principal del sistema fotovoltaico fuera de red"
    ),
    "What is uninterruptible power supply (UPS)？": (
        "¿Qué es la fuente de alimentación ininterrumpida (SAI)?"
    ),
    "What is uninterruptible power supply (UPS)?": (
        "¿Qué es la fuente de alimentación ininterrumpida (SAI)?"
    ),
...
ALT_OVERRIDES = {
    ...
    "What is uninterruptible power supply (UPS)ï¼\x9f": (
        "¿Qué es la fuente de alimentación ininterrumpida (SAI)?"
    ),
    "What is uninterruptible power supply (UPS)？": (
        "¿Qué es la fuente de alimentación ininterrumpida (SAI)?"
