#!/usr/bin/env python3
"""Publish xindun-power news page 73 articles to Spanish WordPress (cat 89)."""
...
# Article 1: AC/DC diagram overlays; article 4: working-mode + wiring diagram overlays.
base.DUOMI_MAP = {
    (1, 0): (
        "Translate the diagram overlay labels to Spanish: DC stored in battery, Inverter, "
        "AC power for daily use. Keep the house background, batteries, yellow inverter product, "
        "orange arrows, and product casing labels unchanged."
    ),
    (4, 0): (
        "Translate all diagram labels and legend text to Spanish: PV array, Inverter with solar "
        "controller built in, Solar charge control, Inverter DC/AC, AVR, BATTERY, AC, AC LOAD, "
        "Working mode diagram, (DC) Battery preferred mode, (AC) commercial power preferred mode, "
        "(AC) AC output. Keep schematic layout, colored lines, and box positions unchanged."
    ),
    (4, 1): (
        "Translate diagram labels to Spanish: Solar panels, DC Loads, Battery, Power Grid or "
        "Generator, AC Loads, and the red warning Total load power should not exceed the rated "
        "power of the inverter. Keep inverter rear panel, wiring lines, appliance icons, and "
        "product casing terminal labels unchanged."
    ),
}
...
TITLE_OVERRIDES = {
    ...
    "Is the electricity from the a power storage inverter AC or DC": (
        "¿La electricidad de un inversor de almacenamiento de energía es CA o CC?"
    ),
    "How much is the DC to AC power inverter": (
        "¿Cuánto cuesta el inversor de corriente CC a CA?"
    ),
    "DC 12v to AC 220v 1000w inverter power inverter charger for car": (
        "Cargador inversor de corriente CC 12 V a CA 220 V 1000 W para coche"
    ),
    "Off grid pure sine wave solar inverter charger with mppt controller built in": (
        "Cargador inversor solar de onda sinusoidal pura fuera de red con controlador MPPT integrado"
    ),
}
...
ALT_OVERRIDES = {
    ...
    "Inverter-with-solar-controller-built-in-2": (
        "Inversor con controlador solar integrado 2"
    ),
}
...
def run_duomi_relaxed_ratio(
    ...
    try:
        raw = best[1]
        cropped = mod.OUT / f"{stem}-cropped.png"
        with Image.open(raw) as im:
            fitted = _fit_crop_to_ratio(im.convert("RGB"), orig_ratio)
            fitted.save(cropped)
        mod.encode_jpeg(cropped, final, orig_wh[0], orig_wh[1], 100)
    except Exception as exc:  # noqa: BLE001
