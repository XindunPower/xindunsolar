#!/usr/bin/env python3
"""Publish xindun-power news page 83 articles to Spanish WordPress (cat 89)."""
...
# All images on this page are product/installation/factory photos without English
# marketing/diagram overlays — compress and rename only (no Duomi).
base.DUOMI_MAP = {}

TITLE_OVERRIDES = {
    "Common faults of solar inverters and their solutions.": (
        "Fallos comunes de los inversores solares y sus soluciones."
    ),
    "How long is the service life of the photovoltaic inverter?": (
        "¿Cuál es la vida útil del inversor fotovoltaico?"
    ),
    "How to improve the power generation efficiency of photovoltaic systems?": (
        "¿Cómo mejorar la eficiencia de generación de energía de los sistemas fotovoltaicos?"
    ),
    "What should we pay attention to when repairing PV inverter?": (
        "¿A qué debemos prestar atención al reparar un inversor fotovoltaico?"
    ),
    "Which manufacturer has better inverter OEM service?": (
        "¿Qué fabricante ofrece un mejor servicio OEM de inversores?"
    ),
}
...
ALT_OVERRIDES = {
    ...
    "inverter OEM manufacturer": "fabricante OEM de inversores",
}
...
def _post_id_from_url(url: str) -> int | None:
    ...
    r = requests.get(
        f"{base.WP}/wp-json/wp/v2/posts",
        params={"slug": slug, "categories": base.CAT, "per_page": 1},
        timeout=30,
    )
    if r.ok and r.json():
...
def main_with_update() -> None:
    ...
    for article_idx, art in enumerate(articles):
        ...
        existing_id = _post_id_from_url(existing) if existing else None
        if existing and not update_existing and os.environ.get("FORCE_PUBLISH") != "1":
        ...
        for p in prepared:
            if existing_id and update_existing and p["local"].exists():
        ...
        if existing_id and update_existing:
            r = wp.s.post(
                f"{base.WP}/wp-json/wp/v2/posts/{existing_id}",
            ...
            if r.status_code >= 400:
                raise RuntimeError(f"update post failed {r.status_code}: {r.text[:500]}")
            post = r.json()
            print(f"[post] updated id={post['id']} {post.get('link')}", flush=True)
            status = "updated"
        else:
