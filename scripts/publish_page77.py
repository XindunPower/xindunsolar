#!/usr/bin/env python3
"""Publish xindun-power news page 77 articles to Spanish WordPress (cat 89)."""
...
# Article 1: hand-drawn PV/inverter schematic (Utility Grid, LOADS, Inverter, PV).
# Article 2: showroom with English marketing posters on back wall.
# Article 3: marketing graphic with Inverter/UPS overlay labels.
# Article 4: DC/AC waveform diagram with English captions.
# Article 0: product photo (compress+rename only).
base.DUOMI_MAP = {
    (1, 0): (
        "Translate diagram labels to Spanish: Utility Grid, 120/240V LOADS, Inverter, PV, "
        "DC-DC PC. Keep transformer taps H1 H2 X1 X2 X3, voltage numbers, switch labels SW1-SW4, "
        "component symbols, colored lines, and hand-drawn layout unchanged."
    ),
    (2, 0): (
        "Translate English marketing poster text on the back wall to Spanish "
        "(e.g. Off-grid Solar Inverter, Hybrid Solar Inverter, Lithium Battery). "
        "Keep product photos, XINDUN logos on products, Chinese text, showroom layout, "
        "display stands, and composition unchanged."
    ),
    (3, 0): (
        "Translate overlay labels to Spanish: Inverter (misspelled Inverer) and UPS. "
        "Keep product casings, LCD screens, futuristic background, and layout unchanged."
    ),
    (4, 0): (
        "Translate diagram labels to Spanish: DC, AC, Inverter, "
        "'The current magnitude and direction are the same', "
        "'The current magnitude and direction change periodically'. "
        "Keep waveform graphs, axes v and t, orange arrows, inverter product photo, "
        "and black background layout unchanged."
    ),
}

TITLE_OVERRIDES = {
    "100kw a/c solar power inverters converters for solar pv power systems": (
        "Convertidores de inversores de energía solar de 100 kW a/c para sistemas fotovoltaicos"
    ),
    "12v 220v pure sine wave power inverter schematic": (
        "Esquema del inversor de potencia de onda sinusoidal pura de 12 V 220 V"
    ),
    "What is a home use solar power inverter used for?": (
        "¿Para qué se utiliza un inversor de energía solar para uso doméstico?"
    ),
    "What is the difference between a UPS and an inverter?": (
        "¿Cuál es la diferencia entre un UPS y un inversor?"
    ),
    "How do I invert 12v to 220v?": (
        "¿Cómo invierto 12 V a 220 V?"
    ),
}
