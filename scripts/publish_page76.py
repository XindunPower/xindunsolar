#!/usr/bin/env python3
"""Publish xindun-power news page 76 articles to Spanish WordPress (cat 89)."""
...
# Article 1: bidirectional energy storage diagram (Power grid, Battery, Inverter, AC loads).
# Article 2: hybrid solar wiring diagram (AC Input/Output, Power Grid or Generator, etc.).
# Articles 0, 3, 4: product photos (compress+rename only).
base.DUOMI_MAP = {
    (1, 0): (
        "Translate diagram labels to Spanish: Power grid, Battery, Inverter, AC loads, "
        "AC, DC. Keep transmission towers, inverter product, batteries, household appliances, "
        "arrows, icons, and layout unchanged."
    ),
    (2, 0): (
        "Translate diagram labels to Spanish: Communication port RS232/RS485 (Optional), "
        "AC Input, AC Output, Power Grid or Generator, AC Loads, Battery. "
        "Keep inverter product, circuit breaker, generator, appliances, wiring lines, "
        "and composition unchanged."
    ),
}

TITLE_OVERRIDES = {
    "How\u00a0does\u00a0inverter\u00a0power inverter\u00a0storage battery\u00a0pack work?": (
        "¿Cómo funciona el paquete de batería de almacenamiento del inversor de corriente?"
    ),
    "How does inverter power inverter storage battery pack work?": (
        "¿Cómo funciona el paquete de batería de almacenamiento del inversor de corriente?"
    ),
    "What is the meaning of bidirectional energy storage inverters?": (
        "¿Cuál es el significado de los inversores bidireccionales de almacenamiento de energía?"
    ),
    "How to connect a 5 kw hybrid solar off grid inverter?": (
        "¿Cómo conectar un inversor solar híbrido fuera de red de 5 kW?"
    ),
    "How to connect a 5 kw hybrid solar off grid inverter\uff1f": (
        "¿Cómo conectar un inversor solar híbrido fuera de red de 5 kW?"
    ),
    "Best solar power ac inverter for car with battery storage": (
        "Mejor inversor de corriente alterna solar para coche con almacenamiento de batería"
    ),
    "The importance of industrial pure sine wave inverter to industry": (
        "La importancia del inversor industrial de onda sinusoidal pura para la industria"
    ),
}
