#!/usr/bin/env python3
"""Publish xindun-power news page 74 articles to Spanish WordPress (cat 89)."""
...
# Article 0: price table overlay; article 1: DC/3-phase diagram; article 3: marketing banner;
# article 4: waveform comparison labels. Article 2: team-building photos (compress only).
base.DUOMI_MAP = {
    (0, 0): (
        "Translate the marketing overlay text to Spanish: Rated Power, Price, and "
        "Contact us for the price of other power of the inverter. Keep the price table "
        "numbers, three yellow inverter products, dark background, and layout unchanged."
    ),
    (1, 0): (
        "Translate diagram labels to Spanish: DC, Three phase inverter, 3 phase AC. "
        "Keep solar panel grid, black inverter products, colored arrows, and three-phase "
        "waveform graph unchanged."
    ),
    (3, 0): (
        "Translate marketing overlay text to Spanish: NB series pure sine wave inverter, "
        "Output pure alternating current, Compatible with all electrical appliances. "
        "Keep the Xindun brand name, all inverter products, red banner layout, and background."
    ),
    (4, 0): (
        "Translate waveform diagram labels to Spanish: Square wave, Modified sine wave, "
        "Pure sine wave. Keep oscilloscope, inverter product, waveform curves, and layout unchanged."
    ),
}

TITLE_OVERRIDES = {
    "Off grid solar a/c inverters prices in pakistan": (
        "Precios de inversores solares CA fuera de red en Pakistán"
    ),
    "3 phase solar power battery storage inverter charger for sale": (
        "Cargador inversor de almacenamiento de batería solar trifásico en venta"
    ),
    "Xindun Power 2020 team building activity-outdoor barbecue": (
        "Actividad de team building de Xindun Power 2020: barbacoa al aire libre"
    ),
    "Why can't square wave inverter carry inductive load?": (
        "¿Por qué el inversor de onda cuadrada no puede alimentar cargas inductivas?"
    ),
    "Pure sine wave inverter vs. square wave inverter vs modified sine wave": (
        "Inversor de onda sinusoidal pura vs. inversor de onda cuadrada vs onda sinusoidal modificada"
    ),
}
