#!/usr/bin/env python3
"""Publish xindun-power news page 55 articles to Spanish WordPress (cat 89)."""
...
# Images on this page that contain English marketing/diagram overlays.
# Other images are factory/product shots and must stay original (compress+rename only).
DUOMI_MAP: dict[tuple[int, int], str] = {
    (0, 0): (
        "Translate the waveform labels in the top diagrams to Spanish. "
        "Keep chart lines, inverter product, oscilloscope, and layout unchanged."
    ),
    (1, 0): (
        "Translate diagram labels to Spanish: power grid, generator, solar panels, "
        "hybrid solar inverter, batteries, AC loads, household devices, office equipment, "
        "industrial equipment, and DC loads. Keep all arrows, icons, product images, "
        "and composition unchanged."
    ),
    (2, 0): (
        "Translate the marketing poster text to Spanish while preserving all products, "
        "background solar panels, badges, and layout. Do not redesign."
    ),
    (2, 1): (
        "Translate the 'factory display' overlay text to Spanish. "
        "Keep workers, machines, workshop photos, and composition unchanged."
    ),
}
...
def unique_stem(base: str) -> str:
    stem = base
    n = 2
    while stem in USED_FILE_STEMS:
        stem = f"{base}-{n}"
        n += 1
...
def encode_jpeg(src: Path, dest: Path, tw: int, th: int, max_kb: int = 100) -> None:
    """Encode JPEG under size cap while keeping original dimensions."""
    ...
    for quality in [92, 88, 84, 80, 74, 68, 62, 56, 50, 44, 38, 32, 28, 24, 20, 16, 12, 10]:
        buf = BytesIO()
        ...
        if len(data) <= max_kb * 1024:
            ...
            return
    raise RuntimeError(f"cannot keep {dest.name} <= {max_kb}KB without changing dimensions")
...
def compress_only(local: Path, stem: str, orig_wh: tuple[int, int]) -> Path:
    dest = OUT / f"{stem}.jpg"
...
def run_duomi(
    ...
    for attempt in range(attempts):
        ...
        try:
            ...
            # Large visual drift usually means poster redesign; reject and retry.
            if diff_score > 28.0:
...
def fetch_article(url: str) -> dict:
    ...
    h1 = soup.find("h1")
    title_en = clean_spaces(h1.get_text(" ", strip=True) if h1 else "")
    seo_title = clean_spaces(
        (
    ...
    description_en = clean_spaces(desc_meta.get("content", "") if desc_meta else "")
...
class WPClient:
    ...
    def create_post(self, title: str, description: str, content: str, featured_media: int) -> dict:
        last_err = ""
        for attempt in range(6):
            r = self.s.post(
...
def prepare_images(article_idx: int, images: list[dict]) -> list[dict]:
    prepared = []
    for img_idx, im in enumerate(images):
        ...
        stem = unique_stem(slugify(alt_es))
        ...
        tagged = OUT / f"{stem}.jpg"
        method = "duomi" if (article_idx, img_idx) in DUOMI_MAP else "compress"
        duomi_path = None
        if tagged.exists() and tagged.stat().st_size > 0:
        ...
        elif (article_idx, img_idx) in DUOMI_MAP:
            size = size_for(*orig_wh)
            result = run_duomi(
                im["src"],
                DUOMI_MAP[(article_idx, img_idx)],
                stem,
                size,
                orig_wh,
                local,
            ...
            final = compress_only(local, stem, orig_wh)
...
def main() -> None:
    ...
    for article_idx, art in enumerate(articles):
        ...
        title_en = clean_spaces(raw["title_en"] or art["title_en"])
        seo_title_en = clean_spaces(raw["seo_title_en"] or title_en)
        desc_en = clean_spaces(raw["description_en"] or title_en)

        title_es = translate_text(title_en)
        seo_title_es = translate_text(seo_title_en)
