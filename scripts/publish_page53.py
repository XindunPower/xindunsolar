...
DUOMI_MAP = {
    (0, 1): (
        "Translate comparison-chart overlay labels to Spanish only. "
        "Keep controller photos, table/grid lines, icons, and all numbers/spec values unchanged."
    ),
    (1, 0): (
        "Translate any marketing/diagram overlay text to Spanish only. "
        "Keep waveform lines, bullets, numbering, product photos, and technical values unchanged."
    ),
    (2, 1): (
        "Translate three-phase diagram labels to Spanish only, including labels like "
        "'Phase 1/2/3' to 'Fase 1/2/3' when present. "
        "Keep sine-wave shapes, angle marks, wiring, and all values unchanged."
    ),
    (3, 0): (
        "Translate poster overlay text to Spanish only. Keep inverter photo, connectors, logos, "
        "spec numbers, and panel layout exactly unchanged."
    ),
    (3, 1): (
        "Translate poster overlay text to Spanish only. Keep inverter photo, icons, logos, "
        "spec values, and composition unchanged."
    ),
    (3, 2): (
        "Translate wiring-diagram labels to Spanish only. Keep device photos, line routes, arrows, "
        "terminal names/model numbers, and all electrical values unchanged."
    ),
    (3, 4): (
        "Translate wiring/connection labels to Spanish only. Keep all wiring paths, voltage/current values, "
        "terminal markings, and product photos unchanged."
    ),
    (3, 5): (
        "Translate infographic/question-card overlay text to Spanish only. "
        "Keep icons, background, layout, and any product/company marks unchanged."
    ),
}
...
def size_for(w: int, h: int) -> str:
    ...
    def n16(v: float) -> int:
        return max(256, int((v + 8) // 16) * 16)

    candidates: list[tuple[float, float, int, int]] = []
    min_pixels = 655_360
    max_pixels = 8_294_400

    min_h = n16(max(256, h * 0.7))
    max_h = min(2048, n16(max(h * 3.2, 1024)))

    for hh in range(min_h, max_h + 1, 16):
        ww = int(round((ratio * hh) / 16.0)) * 16
        if ww < 256 or ww > 2048:
            continue
        pixels = ww * hh
        if pixels < min_pixels or pixels > max_pixels:
            continue
        rdiff = abs((ww / hh) - ratio)
        scale_penalty = abs((ww / w) - 1.0) + abs((hh / h) - 1.0)
        candidates.append((rdiff, scale_penalty, ww, hh))
    ...
    for threshold in (0.01, 0.02, 0.03, 0.05):
        ...
        if valid:
            _, _, ww, hh = min(valid, key=lambda c: (c[1], c[0]))
            return f"{ww}x{hh}"

    _, _, ww, hh = min(candidates, key=lambda c: (c[0], c[1]))
    return f"{ww}x{hh}"
